...
export const ManageProducts = ({categories = []}) => {
  ...
  const handleConfirmDelete = async () => {
    try {
      ...
      setProducts((prevState) => prevState.filter((product) => product.productId !== productToDelete));
  ...
  const handleSaveEdit = async (updatedProduct) => {
    ...
    try {
      ...
      const updatedProductObj = Array.from(updatedProduct.entries()).reduce((obj, [key, value]) => {
        obj[key] = value;
        return obj;
      }, {});
      setProducts((prevState) =>
        prevState.map((product) =>
          product.productId === updatedProductObj.productId ? updatedProductObj : product
        )
      );
      const data = await getProducts();
      ...
      {error ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {products.map((product) => (
            <div key={product.productId} className="border rounded-lg p-4 shadow-lg">
              <div className="flex items-center">
                <img src={`data:image/jpeg;base64,${product.image}`} alt={product.name} className="h-32 w-32 object-cover rounded-lg shadow-sm mr-4" />
                <div>
